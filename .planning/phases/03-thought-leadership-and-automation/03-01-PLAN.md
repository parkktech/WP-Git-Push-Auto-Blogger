...
must_haves:
  ...
  artifacts:
    - path: "scripts/generate-thought-leadership.js"
      ...
      exports: ["getISOWeekNumber", "selectPillarAndAngle"]
      min_lines: 150
  key_links:
...
The `module.exports` line exposes `getISOWeekNumber` and `selectPillarAndAngle` for Task 2's determinism verification (and future testing). This follows the same pattern as `evaluate-commit.js`, which exports `POST_JSON_SCHEMA` alongside its own `main()` entry point.
