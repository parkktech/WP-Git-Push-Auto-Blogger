...
Note on SEO meta: Research confirms WordPress silently drops unregistered meta fields (no error). Writing both Yoast and RankMath fields when `plugin === 'both'` is safe — only the registered fields will persist. The Phase 4 WordPress plugin handles `register_post_meta()` with `show_in_rest: true`. For Phase 2 testing, a minimal PHP stub may be needed (see research Open Question 1).
  </action>
  <verify>
